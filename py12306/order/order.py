...
class Order:
    ...
    def make_passenger_ticket_str(self):
        ...
        logger.info("Start sumbit order request")
        for passenger in available_passengers:
            tmp_str = '{seat_type},0,{passenger_type},{passenger_name},{passenger_id_card_type},{passenger_id_card},{passenger_mobile},N,{enc_str}_'.format(
                seat_type=self.query_ins['current_order_seat'], passenger_type=passenger['passenger_type'],
                passenger_name=passenger['passenger_name'],
                passenger_id_card_type=passenger['passenger_id_type_code'], passenger_id_card=passenger['passenger_id_no'],
                passenger_mobile=passenger['mobile_no'],
                enc_str=passenger['allEncStr'],
            )
            passenger_tickets.append(tmp_str)

            if int(passenger['passenger_type']) != UserType.CHILD:
    ...
    def request_init_dc_page(self):
        ...
        """
        data = {'_json_att': ''}
        response = self.session.postHtml(API_INITDC_URL, data)
        html = response.text
