...
class Job:
    ...
    def init_data(self, info):
        self.id = info.id
        self.left_dates = info.left_date
        #self.stations = info.stations
        self.stations.append(info.stations) #if isinstance(self.stations, dict) else self.stations
        if not self.job_name:  # name 不能被修改
            self.job_name = '{} -> {}'.format(self.stations[0].split("-")[0], self.stations[0].split("-")[1])

        self.account_key = getattr(info,'account_key')
        self.allow_seats .append( getattr(info,'seats'))
        self.allow_train_numbers = getattr(info,'train_numbers')
        self.except_train_numbers = getattr(info,'except_train_numbers')
        self.members = getattr(info,'members').split(",")
        self.member_num = len(self.members)
        self.member_num_take = self.member_num
        self.allow_less_member = getattr(info,'allow_less_member')
        period = getattr(info,'peroid')
        self.from_time = timedelta(
            hours=int(0), seconds=int(0))
        self.to_time = timedelta(
            hours=int(24), seconds=int(0))
        # if period:
    ...
    def start(self):
        ...
        while True and self.is_alive:
            ...
            self.left_date = self.left_dates
            response = self.query_by_date(self.left_date)
    ...
    def refresh_station(self, station):
        self.left_station = station.split("-")[0]
        self.arrive_station = station.split("-")[1]
