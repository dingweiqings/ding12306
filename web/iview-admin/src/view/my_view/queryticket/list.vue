<template>
  <div>
    ...
    <grabbing  :showModal="showGrabbing" :numArr="numArr" :seatArr="seatArr" :trainInfo="trainInfo" :passengersArr="passengersArr" @closeGrabbingModal="closeGrabbingModal"> </grabbing>
    <c-order @closeOrderModal="closeOrderModal"   :trainInfo="trainInfo" :showModal="showOrder" :seatArr="seatArr" :passengersArr="passengersArr" ></c-order>
  </div>
</template>
...
import grabbing from './c_order_grabbing'
import axios from '@/libs/api.request'
import COrder from './c_order'
import {formateDate} from '@/libs/tools'
import {getAtCurrentDay,getSeatPriceTypes,getSeatArr,hasLeftTicket,getPriceTableData}  from '@/libs/util'
export default {
  name: 'tables_page',
  components: {
    Tables,grabbing,COrder,expandRow,
  },
  data () {
    return {
      total:0,
  ...
  methods: {
    handleExpand(row,status){
      console.log("Expand status",status)
      if(status){
        let index=-1
        this.tableData.forEach((e,i)=>{
          if(e.num===row.num){
        ...
        axios.request({
              ...
              this.priceRow=getPriceTableData(r.data.data.data,row.num)
              console.log("price row",this.priceRow)
      ...
      closeGrabbingModal(){
        ...
        console.log("Show add",this.showGrabbing)
      //  this.loadData()
      },
      showGrabbingMethod(){
        this.showGrabbing=true
        console.log("List Show add",this.showGrabbing)
      },
    handleDelete (params) {
      console.log(params)
    },
